...
module Blurrily
  class Client
    ...
    def initialize(host, port, db_name)
      raise(ArgumentError, "HOST value given was not a valid IP address.") unless host.match(IP_REGEX)
      raise(ArgumentError, "PORT value given was not a valid port value.") unless host.match(PORT_REGEX)
    ...
    def find(needle, limit)
      ...
      raise(ArgumentError, "LIMIT value must be numberic")                 if ref.match(LIMIT_REGEX)

      cmd = ["FIND",@db, needle, limit].join("\t")
    ...
    def put(db_name, needle, ref, weight = 0)
      ...
      raise(ArgumentError, "REF value must be numberic")                   if ref.match(REF_REGEX)
      raise(ArgumentError, "WEIGHT value must be numberic")                if ref.match(WEIGHT_REGEX)
